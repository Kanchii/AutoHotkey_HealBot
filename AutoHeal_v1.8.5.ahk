...
global min_x_lifeBar, min_y_lifeBar, min_x_utilityBar, min_y_utilityBar, ymed_lifeBar
global max_x_lifeBar, max_y_lifeBar, max_x_utilityBar, max_y_utilityBar

global autoPush_x, auto_Push_y, auto_Push_status := 0

global Check1 := false, plife1, key1, x_pos1
global Check2 := false, plife2, key2, x_pos2
global Check3 := false, plife3, key3, x_pos3
global CheckParalyze := false, key_anti_paralyze
...
Gui Principal: Add, ComboBox, x94 y73 w55 +Disabled +Uppercase vcbxKey1 gcbx1_modify, F1|F2|F3|F4|F5|F6|F7|F8|F9|F10|F11|F12
Gui Principal: Add, ComboBox, x94 y110 w55 +Disabled +Uppercase vcbxKey2 gcbx2_modify, F1|F2|F3|F4|F5|F6|F7|F8|F9|F10|F11|F12
Gui Principal: Add, ComboBox, x94 y147 w55 +Disabled +Uppercase vcbxKey3 gcbx3_modify, F1|F2|F3|F4|F5|F6|F7|F8|F9|F10|F11|F12
...
Gui Principal: Font
Gui Principal: Add, CheckBox, x28 y73 w15 h23 vcheckParalyze gCheckParalyzeF
Gui Principal: Add, Text, x50 y73 w65 h23 +Disabled +0x200 vtxtParalyze, Anti-Paralyze:
Gui Principal: Add, ComboBox, x120 y73 w55 +Disabled +Uppercase vcbxKeyParalyze gcbxParalyze, F1|F2|F3|F4|F5|F6|F7|F8|F9|F10|F11|F12
...
^1::
    ...
    if(tipoSelecao = 0){
        ...
        if(status = 1){
            ...
            min_x_lifeBar := min(x1_lifeBar, x2_lifeBar)
            min_y_lifeBar := min(y1_lifeBar, y2_lifeBar)
            max_x_lifeBar := max(x1_lifeBar, x2_lifeBar)
            max_y_lifeBar := max(y1_lifeBar, y2_lifeBar)
            ymed_lifeBar := (min_y_lifeBar + max_y_lifeBar) / 2
            x_pos1 := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * plife1) / 100.0
            x_pos2 := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * plife2) / 100.0
            x_pos3 := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * plife3) / 100.0
...
CheckBox1:
    GuiControlGet, Check1Tmp, Principal:
    if(Check1Tmp = 0){
        Check1 := false
...
CheckBox2:
    GuiControlGet, Check2Tmp, Principal:
    if(Check2Tmp = 0){
        Check2 := false
...
CheckBox3:
    GuiControlGet, Check3Tmp, Principal:
    if(Check3Tmp = 0){
        Check3 := false
...
run:
    ...
    if(Check1){
        ;GuiControlGet, edtLife1, Principal:
        ;life1 := % edtLife1
        ;x_pos := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * life1) / 100.0
        ;y_pos := min_y + ((max_y - min_y) * life1) / 100.0
        ; y_pos := y_med
        PixelGetColor, lifeColor, x_pos1, ymed_lifeBar, RGB
        ;MsgBox, pos = %x_pos1% y = %ymed_lifeBar%
        if(isBlack(lifeColor)){
    ...
    if(Check2){
        ;GuiControlGet, edtLife2, Principal:
        ;life2 := % edtLife2
        ;x_pos := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * life2) / 100.0
        ;y_pos := min_y + ((max_y - min_y) * life2) / 100.0
        ; y_pos := y_med
        ; MsgBox, %x_pos% %y_pos%
        PixelGetColor, lifeColor, x_pos2, ymed_lifeBar, RGB
        if(isBlack(lifeColor)){
    ...
    if(Check3){
        ;GuiControlGet, edtLife3, Principal:
        ;life3 := % edtLife3
        ;x_pos := min_x_lifeBar + ((max_x_lifeBar - min_x_lifeBar) * life3) / 100.0
        ;y_pos := min_y + ((max_y - min_y) * life3) / 100.0
        ;y_pos := y_med
        PixelGetColor, lifeColor, x_pos3, ymed_lifeBar, RGB
        if(isBlack(lifeColor)){
...
/*
    Funções quando ocorre alguma modificação
*/
